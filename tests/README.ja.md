...
ゴールデンが見ているのは `begin()` だけ。**ボタンが仕事をするのは `update()`** で、
そこはバスに何も出さないので 1 行もトレースに出ない。しかもボタンは
**全機種で同じ 1 つのクラス**なので、機種ごとのゴールデンに入れる形にもならない。
だから別に見る。

時刻は `update(msec)` の引数なので、**600 ms の長押しも実時間 0 秒**で、
マシンの速さで結果が変わらない。期待値は**刺激の隣に書いてある**。
スケッチが `output/checks.txt` に書き出し、`FAIL` 行がそのまま assert の
メッセージになる。

生成物ではなく手書き（ボードが出てこないため）。ビルドも実行も約 7 秒なので、
