...
| `("m5pm1_pwm", "Ch0", "Io3", 5000)` | 同じ形で、PWM が電源チップの中にある版。**実装済み** |
...
PWM は M5GFX の 9 bit 整数式、AXP192 は LDO2 / LDO3 / DC3 の電圧ステップ、
M5IOE1 は 12 bit の 2 乗カーブ。**どれも M5GFX と同じ数字になる。**

M5IOE1 の PWM チャネルは**チップ側でピンが固定**（Ch1=IO9 / Ch2=IO8 /
Ch3=IO11 / Ch4=IO10）なので、チャネルとピンの両方を書くのは冗長に見えるが、
**そのピンを出力にしないと PWM が出ない**ので列としては要る。
...
`dsi` は**ピンを 1 本も書かない**（書くとエラー）。代わりに `display_dsi` が要る。

| 列 | 例 |
| --- | --- |
| `display_dsi` | `dict(bus_id=0, lanes=2, mbps=600, ldo_ch=3, ldo_mv=2500,`<br>`dpi_mhz=24, hbp=40, hpw=2, hfp=40, vbp=8, vpw=4, vfp=200)` |

`Display` を広げずに 2 つ目の構造体にした理由は D41。寸法・オフセット・回転は
バスに依らず同じ問いなので `display` に残り、レーンとブランキングだけが分かれる。
...
`busy` は**電子ペーパーだけが持つ**（PaperMono は GPIO18）。書かなければ `-1`。
`begin()` は待たない —— 待つのは描く側の仕事 —— が、**入力にはする**（D39）。
...
### 2.5.1 SD がパネルのバスに載る機種

| 列 | 例 | 使うボード |
| --- | --- | --- |
| `sd_spi_cs` | `4` | Core2 / Tough / CoreS3 / CoreS3SE / StackChan（`10` は StampPLC） |
| | 省略 | カードがパネルのバスに載らない（カードが無い機種を含む） |

**新しい事実は CS 1 本だけ。** バスのピンは `display` に既にある —— 同じバス
だからこの列が要るのであって、別バスなら書かない。

**カードのピンを他に持たない**のは §3 のとおり。`kSdSpiCs` だけがヘッダに出るのは、
内部 I2C や POWER_HOLD と同じで**`begin()` 自身が使うから**（照会は副産物）。

省略した機種でも `kSdSpiCs` は `-1` として出る。`sdToSpiMode()` は `-1` で
何もしないので、**「この機種は該当しない」を実行時に確かめられる**
（`tests/unit/SdSpiMode/` がそれも見ている）。
...
| SD のバス（clk / mosi / miso） | **`display` と同じ**。同じでない機種は列を書かない |
...
| I2C 周波数の既定値 | チップドライバ |
| パネルの初期化コマンド列 | GFX |
| コントローラの GRAM サイズ | 今のところ全機種でドライバの既定値と一致する。食い違う機種が来たら列にする |
