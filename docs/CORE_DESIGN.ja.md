...
5. **電源チップの検出とレール投入**（個体差がある機種はここで名乗らせる）
6. ボード固有の手順（`power_on`）
7. LCD のリセットパルス（ピンの場合）
8. バックライトを既定輝度で点灯

**6 が 5 より後なのは譲れない。** Tough の `power_on` は AXP192 の
IO4 と IO1 を叩くが、チップドライバが繋がる前に呼ぶと**何も起きずに素通りする**。
実装中に一度この順で書いて、ゴールデンに I2C が 1 行も出ないことで気づいた。
