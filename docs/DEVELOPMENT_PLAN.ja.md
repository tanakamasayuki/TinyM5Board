...
**30 機種。ADC / AXP192 / AXP2101 / M5PM1 の 4 系統が、ホストのゴールデンと
...
| `tests/begin/` | **31 スケッチ通過**（Core2 が二択で 2 本）**。群ごとのディレクトリ**で、CI の matrix 軸もこれ。1 バスに複数チップのモデルに対応 |
| `tests/tier0/` | **全機種のヘッダを実物のコアでビルド**。マクロと定数の一致を `static_assert` で（30 機種 + 入口 2 本 / 5 種類の SoC） |
...
**1 スケッチ約 10 秒**（実測）なので 31 本で 5 分台。**群ごとに並列化してある**ので
CI では最長の群の時間で済み、ローカルは `pytest begin/Stick` のように絞れる
（4 機種で約 30 秒 / 実測）。

`tests/tier0` は 32 本ビルドして**約 2 分半**、`tests/unit` は 2 本で**約 16 秒**。
**全部で 8 分台**（29 機種のとき 8 分 16 秒 / ローカル実測）。
