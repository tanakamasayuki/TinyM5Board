...
36 機種 + 入口 2 本で**約 3 分**。SoC は esp32 / S3 / C3 / C6 / H2 / P4 の 6 種類で、
...
SPI は長らく「見えるが誰も触らない」状態だった。**SD のモード落とし（D38）で
実際に流れるようになった**ので、トレースに入れてある。バイトは 1 行 1 バイトではなく
**まとまりで記録する**（16 個の 0xFF が 16 行になると、肝心のコマンド 2 本が埋もれる）。
...
そこで `tests/unit/` を別に置く。いまは 2 本。

| | 何を見るか |
| --- | --- |
| `Button/` | デバウンス・押下と離しのエッジ・長押し・クリック回数（D36）。39 検査 |
| `SdSpiMode/` | SD のモード落とし（D38）。**ゴールデンでは通らない分岐**を見る —— バスが黙っていれば必ず CMD0 を送る側になるので、**既に SPI モードのカード**は答えを差し込まないと作れない。13 検査 |

| | |
| --- | --- |
| 時刻・応答 | **引数とフックで差し込む。** 600 ms の長押しも実時間 0 秒、カードの返事も 1 行 |
| 期待値 | **スケッチの中に、刺激の隣に書く** |
| 落ち方 | `output/checks.txt` の `FAIL` 行がそのまま assert のメッセージになる |
| 共通部分 | `tinym5_expect.h`（報告）と `tinym5_check.check_unit()`（起動と判定） |

**ここはゴールデンにしない。** 凍結が効くのは上流から起こした値
（§1）で、状態機械の期待値は上流を見なくても導ける。
「1110 ms でクリックが立つ」は刺激の隣に書いてあるほうが読める。

1 本あたり約 8 秒。CI では matrix の外に別ジョブで置く。

## 4. Tier 2 —— サンプルのビルド網羅（`tests/tier2/`）

**サンプルは実質のドキュメント**（D20）なので、**読んだ人がそのまま
ビルドできること**を見る。Tier 0 は生成したスケッチを機種ごとに建てるだけで、
**利用者が写すスケッチには触れていない。**

各サンプルが自分の `sketch.yaml` に既定のボードとコアを持っている（D29）ので、
テスト側は建てて結果を見るだけ。4 本で**約 28 秒**。

| | |
| --- | --- |
| `Hello/` | 立ち上げとピンの照会。**画面を使わない**（D20） |
| `Buttons/` | GPIO / PMIC のキー / エキスパンダのピンが**同じ書き方になる**ことの実演 |
| `Battery/` | ADC / AXP192 / AXP2101 / M5PM1 に**同じ質問**をする |
| `Backlight/` | ピン / レール / エキスパンダのチャネルを**同じ `set()`** で |

`kHas*` を見て `#error` で止まるサンプルの検査は**まだ対象が無い**。
いまの 4 本はどれも「無い機能は `#if` で避けて、無いと表示する」形で、
収まるならそちらのほうが良い。止める必要のあるサンプルが出てきたら足す。
...
| 2（`tests/tier2/`） | `esp32:esp32`（profile 固定） | ✅ サンプル 4 本。**matrix の外** |
| 3 | 実機 | ❌ 手動 |
...
uv run pytest begin --profile host          # 全部、37 スケッチで約 6 分
```
